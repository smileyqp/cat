...
/**
 * @brief 外部服务用户中心类
 * @notice 如果需要外部API调用用户权限必须要将会话方式设置成SESSION
 */
class ServiceUcenter
{
    /**
     * @brief 用户登录接口
     * @param $login_info string 用户信息
     * @param $password   string 用户密码
     * @return array
     */
    public function userLogin($login_info,$password)
    {
        $userRow = plugin::trigger("isValidUser",array($login_info,md5($password)));
        if(!$userRow)
        {
            throw new IException("账号或密码错误");
        }
