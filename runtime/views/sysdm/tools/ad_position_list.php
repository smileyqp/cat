...
<div class="content">
	<form name='ad_position' method='post' action='<?php echo IUrl::creatUrl("/tools/ad_position_del");?>'>
		<table class="table list-table">
			...
			<caption>
                <a class="btn btn-default" href='<?php echo IUrl::creatUrl("/tools/ad_position_edit");?>'>
			...
			<tbody>
				<?php $queryObj=Api::run('getListByAdPositionList');$resultData=$queryObj->find()?>
				...
				<tr>
					...
					<td><?php echo $item['fashion']==1 ? '轮回显示':'随机显示';?></td>
					<td><?php echo $item['status']==1 ? '是':'否';?></td>
					<td>
						<a href='<?php echo IUrl::creatUrl("/tools/ad_edit/pid/".$item['id']."");?>'><i class='operator fa fa-plus' title="添加广告数据"></i></a>
						<a href='<?php echo IUrl::creatUrl("/tools/ad_position_edit/id/".$item['id']."");?>'><i class='operator fa fa-edit' title="获取广告位代码"></i></a>
						<a href="javascript:delModel({link:'<?php echo IUrl::creatUrl("/tools/ad_position_del/id/".$item['id']."");?>'});"><i class='operator fa fa-close'></i></a>
					</td>
				</tr>
				<?php }?>
			</tbody>
