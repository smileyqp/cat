...
<div class="content">
	<table id="list_table" class="table list-table">
		...
		<caption>
		<a class="btn btn-default" href='<?php echo IUrl::creatUrl("/tools/article_cat_edit");?>'>
		...
		<tbody>
			...
			<tr id="<?php echo isset($item['id'])?$item['id']:"";?>" parent=<?php echo isset($item['parent_id'])?$item['parent_id']:"";?>>
				<td><i style="margin-left:<?php echo (substr_count($item['path'],',')-2)*2;?>0px" class="operator fa fa-minus-square-o" onclick="displayData(this);" title="关闭"></i><?php echo isset($item['name'])?$item['name']:"";?></td>
				<td><?php echo ($item['issys']==1) ? '是':'否';?></td>
				<td>
					<a href='<?php echo IUrl::creatUrl("/tools/cat_edit/id/".$item['id']."");?>'><i class='operator fa fa-edit'></i></a>
					<a href="javascript:delModel({link:'<?php echo IUrl::creatUrl("/tools/cat_del/id/".$item['id']."");?>'});"><i class='operator fa fa-close'></i></a>
				</td>
			</tr>
			<?php }?>
		</tbody>
