...
<div class="content">
	<table class="table list-table">
		...
		<tbody>
			<?php foreach($items=Api::run('getPaymentAll') as $key => $item){?>
			<tr>
				<td><img src="<?php echo IUrl::creatUrl("")."plugins".$item['logo']."";?>" style='width:130px;height:60px' /></td>
				<td><?php echo isset($item['name'])?$item['name']:"";?></td>
				<td><?php echo isset($item['description'])?$item['description']:"";?></td>
				<td><span class="<?php echo $item['status'] == 0 ? 'text-green' : 'text-red';?>"><?php echo $item['status'] == 0 ? '使用中' : '已关闭';?></span></td>
				<td>
					<a href="<?php echo IUrl::creatUrl("/system/payment_edit/id/".$item['id']."");?>"><i class='operator fa fa-edit'></i></a>
				</td>
			</tr>
			<?php }?>
		</tbody>
