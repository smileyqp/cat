...
<body class="skin-blue fixed sidebar-mini" style="height: auto; min-height: 100%;">
	...
	<ul class="breadcrumb">
		...
		<li class="active">自提点</li>
	</ul>
</div>
<div class="content">
	<form action="<?php echo IUrl::creatUrl("/system/takeself_operate");?>" method="post">
	<table class="table list-table">
		...
		<caption>
            <a class="btn btn-default" href='<?php echo IUrl::creatUrl("/system/takeself_edit");?>'>
                <i class="fa fa-plus"></i>添加自提点
            </a>
		...
		<tbody>
          <?php $queryObj=Api::run('getListByTakeself');$resultData=$queryObj->find()?>
          <?php foreach($items=$resultData as $key => $item){?>
			<tr>
				...
				<td>
					<a href="<?php echo IUrl::creatUrl("/system/takeself_edit/id/".$item['id']."");?>"><i class='operator fa fa-edit'></i></a>
					<a href="javascript:delModel({link:'<?php echo IUrl::creatUrl("/system/takeself_operate/id/".$item['id']."");?>'})"><i class='operator fa fa-close'></i></a>
				</td>
			</tr>
			<?php }?>
		</tbody>
