...
<div class="content">
	<table class="table list-table">
		...
		<caption>
            <a class="btn btn-default" href="<?php echo IUrl::creatUrl("/brand/brand_edit");?>">
		...
		<tbody>
            <?php $queryObj=Api::run('getListByBrand');$resultData=$queryObj->find()?>
            <?php foreach($items=$resultData as $key => $item){?>
			<tr>
				<td><input class="form-control" id="sort<?php echo isset($item['id'])?$item['id']:"";?>" type="text" onblur="toSort(<?php echo isset($item['id'])?$item['id']:"";?>);" value="<?php echo isset($item['sort'])?$item['sort']:"";?>" pattern='int' /></td>
				<td><a href="<?php echo IUrl::creatUrl("/site/brand_zone/id/".$item['id']."");?>" target="_blank"><?php echo isset($item['name'])?$item['name']:"";?></a></td>
				<td><?php echo isset($item['url'])?$item['url']:"";?></td>
				<td>
				<?php foreach($items=explode(',',$item['category_ids']) as $key => $val){?>
				[<?php echo isset($catAll[$val])?$catAll[$val]:"";?>]
				<?php }?>
				</td>
				<td>
                    <a href="<?php echo IUrl::creatUrl("/brand/brand_edit/bid/".$item['id']."");?>"><i class='operator fa fa-edit'></i></a>
                    <a onclick="delModel({link:'<?php echo IUrl::creatUrl("/brand/brand_del/bid/".$item['id']."");?>'})"><i class='operator fa fa-close'></i></a>
                </td>
			</tr>
			<?php }?>
		</tbody>
