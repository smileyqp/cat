	...
	<form action='<?php echo IUrl::creatUrl("/goods/spec_photo_del");?>' method='post' name="specFrom">
		<table class="table list-table">
			...
			<caption>
                ...
                <a class="btn btn-default" onclick="confirm('确定删除么？','formSubmit(\'specFrom\')');">
			...
			<tbody>
                <?php $queryObj=Api::run('getListBySpecPhoto');$resultData=$queryObj->find()?>
                <?php foreach($items=$resultData as $key => $item){?>
				<tr>
					<td><input type="checkbox" name="id[]" value="<?php echo isset($item['id'])?$item['id']:"";?>" /></td>
					<td><img class="img-thumbnail" style="width:60px;height:60px" src="<?php echo IUrl::creatUrl("".$item['address']."");?>" /></td>
