...
<section class="uc_info">
	...
	<div class="user_info">
		...
		<ol class="user_stat">
			<li>预存款余额：<strong>￥<?php echo isset($user['balance'])?$user['balance']:"";?></strong></li>
		...
		<ol class="user_stat">
			<li>待评价商品：(<strong><?php echo statistics::countUserWaitComment();?></strong>)</li>
			<li>待付款订单：(<strong><?php echo statistics::countUserWaitPay();?></strong>)</li>
			<li>待确认收货：(<strong><a href="<?php echo IUrl::creatUrl("/ucenter/order");?>"><?php echo statistics::countUserWaitCommit();?></a></strong>)</li>
		</ol>
	</div>
</section>
...
<section class="uc_table">
	<table>
		...
		<tbody>
		<?php foreach($items=Api::run('getOrderListByUserid') as $key => $item){?>
		<tr>
