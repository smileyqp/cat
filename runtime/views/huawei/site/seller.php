...
<section class="breadcrumb">
	<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> » <a href="<?php echo IUrl::creatUrl("/site/seller");?>">商家列表</a>
</section>
...
<section class="web">
		...
		<section class="seller_list">
			<?php $queryObj=Api::run('getSellerList');$resultData=$queryObj->find()?>
				...
				<ul>
					...
					<li>
						<a href="<?php echo IUrl::creatUrl("/site/home/id/".$item['id']."");?>">
							<h3><?php echo isset($item['true_name'])?$item['true_name']:"";?></h3>
							...
							<div class="star_box">
								...
								<span class="star stat_<?php echo Common::gradeWidth(statistics::gradeSeller($item['id']));?>"></span>
							</div>
							<span>销量：<?php echo statistics::sellCountSeller($item['id']);?> 件</span>
							<address><?php echo join(' ',area::name($item['province'],$item['city'],$item['area']));?></address>
						</a>
					</li>
	...
	<aside class="bar">
		...
		<nav class="bar_box">
			...
			<ul class="bar_nav">
				<?php foreach($items=Api::run('getVipSellerList') as $key => $item){?>
		...
		<div class="bar_box">
			...
			<ul class="bar_goods">
				<?php foreach($items=Api::run('getCommendHot',5) as $key => $item){?>
				<li>
