...
<div class="bigweb">
	...
	<section class="breadcrumb">
		<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> » <a href="<?php echo IUrl::creatUrl("/site/brand");?>">商品品牌</a>
	</section>

	<section class="web">
		<section class="main">
			<?php if(isset($name) && $name != ''){?>
			<h2 class="main_head"><?php echo isset($name)?$name:"";?></h2>
			<div class="brand_list">
				<ul>
					<?php foreach($items=Api::run('getBrandListWhere',array('#id#',$id),100) as $key => $item){?>
					<li>
						<a href="<?php echo IUrl::creatUrl("/site/brand_zone/id/".$item['id']."");?>">
							<img src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['logo']."/w/208/h/120");?>">
			...
			<?php }else{?>
			<h2 class="main_head">推荐品牌</h2>
			<div class="brand_list">
				<ul>
					<?php foreach($items=Api::run('getBrandList',40) as $key => $item){?>
					<li>
						<a href="<?php echo IUrl::creatUrl("/site/brand_zone/id/".$item['id']."");?>">
							<img src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['logo']."/w/208/h/120");?>">
							<strong><?php echo isset($item['name'])?$item['name']:"";?></strong>
		...
		<aside class="bar">
			<nav class="bar_box">
				<h3 class="bar_box_head">品牌分类</h3>
				<ul class="bar_nav">
					<?php foreach($items=Api::run('getBrandCategory',20) as $key => $item){?>
					<li <?php if($item['id']==$id){?>class='current'<?php }?>><a href="<?php echo IUrl::creatUrl("/site/brand/id/".$item['id']."/name/".$item['name']."");?>"><?php echo isset($item['name'])?$item['name']:"";?></a></li>
					<?php }?>
				</ul>
			</nav>
		</aside>
