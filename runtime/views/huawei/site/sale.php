...
<section class="breadcrumb">
	<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> » 特价活动
</section>

<section class="web">
	<?php foreach($items=Api::run('getSaleList') as $key => $promRow){?>
	<h2 class="main_head"><?php echo isset($promRow['name'])?$promRow['name']:"";?></h2>
	<ul class="sale_list">
		<?php foreach($items=$promRow['goodsList'] as $key => $item){?>
		<li>
			<a href="<?php echo IUrl::creatUrl("/site/products/id/".$item['id']."");?>">
				<img src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['img']."/w/250/h/250");?>" alt="<?php echo isset($item['name'])?$item['name']:"";?>">
