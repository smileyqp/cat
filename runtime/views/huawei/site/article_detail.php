...
<section class="breadcrumb">
	<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> > <a href="<?php echo IUrl::creatUrl("/site/article");?>">资讯列表</a> > <?php echo isset($this->articleRow['title'])?$this->articleRow['title']:"";?>
</section>
...
<section class="web">
	<section class="article_main">
		<section class="article article_infor">
			<header>
				<h1 class="article_title"><?php echo isset($this->articleRow['title'])?$this->articleRow['title']:"";?></h1>
				<time class="article_date">发布日期：<?php echo isset($this->articleRow['create_time'])?$this->articleRow['create_time']:"";?></time>
			</header>
			<article class="article_content"><?php echo isset($this->articleRow['content'])?$this->articleRow['content']:"";?></article>
		</section>
		<?php if($this->relationList){?>
		<section class="artice_goods">
			<h3>相关商品</h3>
			<ul>
				<?php foreach($items=$this->relationList as $key => $item){?>
				<li>
					<a href="<?php echo IUrl::creatUrl("/site/products/id/".$item['goods_id']."");?>">
						<img src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['img']."/w/200/h/200");?>" alt="<?php echo isset($item['name'])?$item['name']:"";?>">
	...
	<aside class="article_bar">
		...
		<nav class="news_article_box">
			...
			<ul class="news_article_infor">
				<?php foreach($items=Api::run('getArticleCategoryList',10) as $key => $item){?>
