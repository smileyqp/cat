...
<div class="bigweb">
	<section class="breadcrumb">
		<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> » <?php echo isset($this->cat_row['name'])?$this->cat_row['name']:"";?>
	</section>

	<section class="web">
		<section class="main">
			<h2 class="main_head"><?php echo isset($this->cat_row['name'])?$this->cat_row['name']:"";?></h2>
			<div class="article_list">
				<ul>
					<?php foreach($items=$this->helpQuery->find() as $key => $item){?>
					<li><time><?php echo date('Y-m-d H:i:s',$item['dateline']);?></time><a href="<?php echo IUrl::creatUrl("/site/help/id/".$item['id']."");?>"><?php echo isset($item['name'])?$item['name']:"";?></a></li>
					<?php }?>
				</ul>
				<?php echo $this->helpQuery->getPageBar();;?>
			</div>
		</section>
		<!-- 文章详情侧边 -->
		<aside class="article_bar">
			<!-- 帮助侧栏 -->
			<?php foreach($items=Api::run('getHelpCategoryLeft',10) as $key => $item){?>
			<nav class="news_article_box">
				<h3 class="news_article_box_head"><?php echo isset($item['name'])?$item['name']:"";?></h3>
				<ul class="news_article_infor">
				<?php foreach($items=Api::run('getHelpListByCatidAll',array('#cat_id#',$item['id'])) as $key => $help_row){?>
