...
<section class="breadcrumb">
	<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> > 商城公告
</section>

<section class="web">
	<section class="article_main">
		<h2 class="main_head">公告列表</h2>
		<div class="article_list">
			<ul>
				<?php $queryNotice = Api::run('getNoticeList')?>
				<?php $resultData = $queryNotice->find()?>
				<?php if($resultData){?>
					<?php foreach($items=$resultData as $key => $item){?>
					<li><time><?php echo isset($item['time'])?$item['time']:"";?></time><a href="<?php echo IUrl::creatUrl("/site/notice_detail/id/".$item['id']."");?>"><?php echo isset($item['title'])?$item['title']:"";?></a></li>
					<?php }?>
				<?php }else{?>
				<p class="no_tip">暂无相关信息</p>
				<?php }?>	
			</ul>
			<?php echo $queryNotice->getPageBar();?>
		</div>
	</section>
	<!-- 文章详情侧边 -->
