...
<section class="breadcrumb">
	<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> » 全部商品分类
</section>
...
<section id="sitemap" class="sitemap">
	...
	<section class="sitemap_list">
		...
		<div class="sitemap_class">
			<a name="class_<?php echo isset($key)?$key:"";?>"></a>
			...
			<dl>
				...
				<dd>
					<?php foreach($items=Api::run('getCategoryByParentid',array('#parent_id#',$second['id'])) as $key => $third){?>
					<?php if($key!=0){?> | <?php }?><a href='<?php echo IUrl::creatUrl("/site/pro_list/cat/".$third['id']."");?>'><?php echo isset($third['name'])?$third['name']:"";?></a>
					<?php }?>
				</dd>
			</dl>
			<?php }?>
